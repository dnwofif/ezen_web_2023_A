		...
		$.ajax({
			...
			success : function f(r){ 
				...
				for (let i=0; i<r.length; i++){
					...
					html += `<button onclick = select(${i}) class="unusedselect seat">${i+1}</button>`;
					info[i] = false;
					} 
					else if (r[i].state >0 ) {
					html += `<button onclick = select(${i}) class="usedseat seat">X</button>`;
					info[i] = false;
					} 
					else if(i==click){
					html += `<button onclick = select(${i}) class="selectbutton seat">${i+1}</button>`;
					}
					else {
						html += `<button onclick = select(${i}) class="unusedseat seat">${i+1}</button>`;
