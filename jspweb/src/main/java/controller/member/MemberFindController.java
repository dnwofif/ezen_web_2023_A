...
/*
	...
	로그인 했다는 증거 => 인증 [인증서 -> 세션 저장]
		...
		JS :  sessionStorage	: 모든 JS에서 사용가능한 메모리 공간
			세션타입 : 문자열 타입	( 타입변환 필수! )
			세선저장 : sessionStrorage.setItem(키, 데이터);
			세선호출 : sessionStorage.getItem(키);
			
		JAVA(서블릿) : request.getSession() : 모든 서블릿에서 사용가능한 메모리 공간
			세선타입 : Object		( 타입변환 필수! )
			세선저장 : request.getSession().setAttribute("속성명","데이터");
			세선호출 : request.getSession().getAttribute("속성명");
			
	서블릿
		내장객체
			request : 요청객체
				request.getParameter() : ajax 으로 부터 전달된 데이터 요청
