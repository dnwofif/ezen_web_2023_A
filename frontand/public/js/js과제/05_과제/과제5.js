...
document.body.addEventListener('keydown' , ( e )=>{
	...
	if( e.keyCode == 37 ){
		...
		u_left += 10; 	// 10증가
		// * 유효성검사 [ 배경 밖을 못나가게]
		if (u_left > 900){u_left=900};	//만약에 위치가 900보다 크면 900으로 고정
		
		userbox.style.backgroundImage  = `url(img/캐릭터1_이동.png)`;	// 이동 이미지 출력
		
	}else if( e.keyCode == 65 ){	// a키 눌렀을 때
		console.log('공격');
		userbox.style.backgroundImage = `url(img/캐릭터1_공격.png)`;		// 공격 이미지 출력
	}
	//3.현재 위치값을 대입
