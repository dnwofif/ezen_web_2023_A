/*
	...
	- 상수 : 데이터 1개 저장하는 메모리 공간
		...
		- 고정된 값, 변하지 않는값, 일정한값 [파이(3.14) 등등 ]
		
	- 변수/상수 형태
		1. 키워드 [let vs const]
		2. 변수/상수 이름 [영문, 띄어쓰기x, 숫자시작x, 카멜표기법 권장]
			카멜표기법 claswname --> className
		3. 대입연산자 =
			오른쪽 데이터가 왼쪽에 대입/저장
		4. 데이터
	...
	- 연산자	[연산/계산 시 사용되는 특수문자 ]
		...
		3. 비교연산자 : true 혹은 false 출력
			...
			!=같지않다/아니다	 !== 같지않다/아니다
		4. 관계연산자 : 주로 비교연산자가 2개이상일 때 사용, true 혹은 false 출력
		...
		5. 대입연산자 : 
			...
			+= [오른쪽 데이터를 왼쪽에 더한 후에 왼쪽에 대입]
				1. 변수 = 10
				2. 변수 += 30		[+= 1.더하기를 한다 (30+10 => 40)]
				3. console.log(변수)
			-= *= /= %=
		6. 증감연산자 :
			++ 1증가 [선위증가 : ++변수	/ 후위증가 : 변수++]
			-- 1감소 [선위감소 : --변수	/ 후위감소: 변수--"] 
		7. 삼항연산자 :  항3개 존재하는 연산자, 참 또는 거짓 출력
			조건 ? 참true : 거짓 false
			- 만약에 조건이 true이면 참 실행
...
let 변수1 = 10//'변수1'변수에 10 데이터를 저장한다
...
//4. 변수의 연산
let 변수2 = 변수1 + 10 	// 변수 2 = 20+10 	// 변수명 호출시 변수안에 있는 데이터 출력
console.log(변수2)		// 변수 2 = 30
console.log(변수2+10)		// 변수 2+10 => 40
