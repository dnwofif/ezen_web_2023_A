/*
	함수[함 : 상자 / 수 : 숫자]
		...
		2. 하나의 특별한 목적의 작업을 수행하기 위한 *코드의 집합*
		3. 사용 목적
			1. 재활용성/재사용 : 한번 정의[만들면]하면 반복적으로 사용 가능	/ 중복 코드 묶음처리 = 클린코딩
		...
		4. 함수선언
			function 함수명 ( 인수1 , 인수2 , 인수3 ){
				...
				5. return : 함수 종료/리턴		[*해당 함수를 호출했던 곳으로 반환/리턴 ]
					...
					return;		반환 값이 없으므로 반환값 없이 함수종료
				ex) 1. 유재석이 핸드폰 고장나서 서비스센터 간다.
					유재석				삼성 서비스센터(함수)
						----고장난핸드폰--->	
						...
						<----수리된핸드폰----
							(반환/리턴)
						1. 함수 정의
						function 서비스센터 (고장난핸드폰){
							핸드폰기사 핸드폰수리 코드
							return 수리된핸드폰
						}
						2. 함수 호출	
						let 유재석 = 서비스센터 (유재석핸드폰, 1000)
						let 강호동 = 서비스센터 (강호동핸드폰, 10000)
...
function 함수4(x , y, z){return 3;}	//4. 인수 있다 / 반환 값 없다.	-> 함수종료역할
function 함수5(){return 5;}			//5. 인수 없다 / 반환 값 없다.	-> 함수종료역할
function 함수6(){return;}				//5. 인수 없다 / 반환 값 = 단순 함수종료 역할
	// * 함수가 종료되는 조건 : 1. '}'가 끝났을 때 2. return 키워드를 만났을때 [아래 코드 있어도 종료!]
	
//2. 함수 호출
console.log('안녕!!')					//1. 미리 정의된(JS 만든 사람) log(인수=출력하고싶으문자열)함수
배열명.splice(0,1)					//2. 미리 정의된(JS 만든 사람) splice(인수1, 인수2)
document.querySelector('')			//3. 미리 정의된(JS 만든 사람) querySelector(인수1 = ???)
			//-----> 다른 사람이 만든(API, 프레임워크, 라이브러리)코드는 설계 모르기 때문에 암기 X
함수명1();							//4. js 에서 내가 만든 함수 호출
함수명2(3);							//5. 함수호출과 동시에 해당 함수에 3 전달 [ x = 3 ]
함수명3(3, 5, 20)						//6. 함수호출과 동시에 해당 함수에게 데이터 3개 전달 [ x = 3, y = 5, z = 20]
let 함수결과1 = 함수명3(3, 5, 20)		//7. 함수 종료후 리턴/반환 값을 변수에 저장
