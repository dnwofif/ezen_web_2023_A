...
public class part1_3 {
	public static void main(String[] args) {
		...
		while(true) {
			...
			if(ch==1) 
				{if(콜라재고>0) {콜라바구니++;콜라재고--;System.out.println("안내) 콜라를 담았습니다.");
				}else System.out.println("안내) 콜라 재고가 없습니다. [관리자에게 문의]");
			}else if(ch==2) 
				{if(환타재고>0) {환타바구니++;환타재고--;System.out.println("안내) 환타를 담았습니다.");
				}else System.out.println("안내) 환타 재고가 없습니다. [관리자에게 문의]");
			}else if(ch==3) 
				{if(사이다재고>0) {사이다바구니++;사이다재고--;System.out.println("안내) 사이다를 담았습니다.");
				...
				if(ch2==1) {
					...
					콜라재고+=콜라바구니; 콜라바구니=0; 환타재고+=환타바구니; 환타바구니=0; 사이다재고+=사이다바구니; 사이다바구니=0;
					System.out.println("안내)장바구니를 초기화했습니다. 초기메뉴로 돌아갑니다.");
				}
			}
