...
public class Board {
	...
	//1. 필드 : 객체의 데이터 저장하는 곳
		/*
		 	- 필드 종류
			 	...
			 	- 정적필드		: (메소드영역)[static] 프로그램 실행시 우선할당 객체없이 저장되어 있는 필드
			
			- 필드 타입
				- 기본타입		: boolean char byte short int long float double 8가지
				- 참조타입		: 클래스[객체], 인터페이스, 배열, 열기타입 등등
		 ...
		 */
	//2. 생성자 : 객체 초기화(처음값대입) 할때 사용되는 메소드
		/*
		 	- 생성자 사용되는 경우 :  객체 선언할때
		 		클래스명 객체변수명 = new 생성자명();
		 	...
		 	- 생성자 종류 [오버로딩으로 인해 다양하게 가능]
		 		- 오버로딩 : 매개변수의 개수,순서,타입에 따라서 식별 가능한 것
		 	
		 	- 접근제한 private, public, default
		 
		 	- 기타 키워드
		 		- this.필드명 	: 외부로부터 들어온 매개변수명과 현재 클래스내 필드명과 같을때 필드 식별할때
		 		- this()		: 현재 클래스의 생성자 호출
	...
	/*
	 	...
	 	- MVC 디자인 패턴 : 기능역할에 따른 구분
	 		...
	 		C : 제어/로직/유효성검사 등등 담당
	 			- 객체를 여러개 가질 필요가 없다. 회원가입기능 1개만 있어도 가능하니까
	 			- 동일한 메소드를 갖는 객체가 있어도 되지만 필요없다.
